...
export const MotionDatabase = () => {
    ...
    async function loadMotions() {
        if (configJustChanged) {
            ...
            preliminaryMotions.sort((a, b) => (a.data().round > b.data().round) ? 1 : -1)
            ...
            motionDataSorted.forEach(async (doc) => {
                ...
                setMotionData([...motionData, { ...doc.data(), topicList: tempTopics, tournament: name, year: year }])
            })
            setLoading(false)
    ...
    return (
        <div className={`motionDatabase ${motions == [] ? 'notLoaded' : ''}`}>
            ...
            <button id="searchMotionsButton" onClick={() => { loadMotions() }}>Search</button>
            <div style={loading ? { display: "flex", margin: "1rem", width: "100%", justifyContent: "center" } : { display: "none" }}>Loading</div>
                            ...
                            render: (motion) => {
                                return (
                                    <TextareaAutosize value={
                                        `${motion.tournament} ${motion.year} (Round: ${motion.round})`} disabled={true} style={{
            ...
            <div className="submitNewMotionNotification">
                ...
                <a href="/new_motion"><button>Submit it</button></a>
            </div>
        </div>
